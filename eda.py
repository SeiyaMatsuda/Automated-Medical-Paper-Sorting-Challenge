...
def set_eda(df_, alpha_sr=0.05, alpha_ri=0.0, alpha_rs=0.0, p_rd=0.1, num_aug=16):
    df = df_[df_.judgement==1]
    new_lines = []
    eda_df = pd.DataFrame()
    for line, label in tqdm.tqdm(zip(df['text'].tolist(), df['judgement'].tolist()), total=len(df['judgement'].tolist())):
        new_line = eda(line, alpha_sr=alpha_sr, alpha_ri=alpha_ri, alpha_rs=alpha_rs, p_rd=p_rd, num_aug=num_aug)
        for l in new_line:
